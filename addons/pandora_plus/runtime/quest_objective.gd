...
func add_progress(delta: int) -> void:
	if _is_completed or not _is_active:
		return
...
func reset() -> void:
	_current_progress = 0
	_is_completed = false
	_is_active = not _quest_objective_instance.is_sequential() or _quest_objective_instance.get_order_index() == 0
...
func get_display_text() -> String:
	...
	if _quest_objective_instance.is_optional():
		text += " (Optional)"
