...
func _init(p_quest: Variant = null) -> void:
	if p_quest is PPQuest:
		...
		if extension_configuration:
			...
			quest_data = p_quest.save_data(fields_settings)
		else:
			...
			quest_data = {
				...
				"level_requirement": p_quest.get_level_requirement(),
				"auto_complete": p_quest.is_auto_complete(),
				"time_limit": p_quest.get_time_limit(),
...
func get_time_limit() -> float:
	return quest_data.get("time_limit", 0.0)

func get_time_remaining() -> float:
	var limit = get_time_limit()
	if limit <= 0:
		return -1.0  # No time limit
	return max(0.0, limit - _time_elapsed)

func is_timed() -> bool:
	return get_time_limit() > 0.0
...
func get_level_requirement() -> int:
	return quest_data.get("level_requirement", 1)
...
func get_required_objectives_count() -> int:
	var count = 0
	for objective in _runtime_objectives:
		if not objective._quest_objective_instance.is_optional():
			count += 1
	return count
...
func get_completed_required_objectives_count() -> int:
	var count = 0
	for objective in _runtime_objectives:
		if not objective._quest_objective_instance.is_optional() and objective.is_completed():
			count += 1
	return count
...
func start() -> void:
	...
	# Activate first objectives (non-sequential or first sequential)
	for i in range(_runtime_objectives.size()):
		var objective = _runtime_objectives[i]
		if not objective._quest_objective_instance.is_sequential():
			objective.set_active(true)
			objective_activated.emit(i)
		elif objective._quest_objective_instance.get_order_index() == 0:
			objective.set_active(true)
			objective_activated.emit(i)
...
## Update (call from game loop if timed quest)
...
func update(delta: float) -> void:
	...
	# Check time limit
	if is_timed() and _time_elapsed >= get_time_limit():
		time_limit_expired.emit()
		fail()
...
func add_objective(runtime_objective: PPRuntimeQuestObjective) -> void:
	...
	runtime_objective.completed.connect(_on_objective_completed.bind(_runtime_objectives.size() - 1))
...
func find_objective_by_id(objective_id: String) -> PPRuntimeQuestObjective:
	for objective in _runtime_objectives:
		if objective._quest_objective_instance.get_objective_id() == objective_id:
			return objective
	return null
...
static func from_dict(data: Dictionary) -> PPRuntimeQuest:
	...
	runtime._time_elapsed = data.get("time_elapsed", 0.0)
...
func get_display_text() -> String:
	...
	if is_timed() and is_active():
		var remaining = get_time_remaining()
		text += " [%ds left]" % int(remaining)
...
func _on_objective_completed(objective_index: int) -> void:
	...
	# Activate next sequential objective if applicable
	if objective_index < _runtime_objectives.size():
		var completed_obj = _runtime_objectives[objective_index]
		if completed_obj._quest_objective_instance.is_sequential():
			var next_index = completed_obj._quest_objective_instance.get_order_index() + 1

			# Find and activate next sequential objective
			for i in range(_runtime_objectives.size()):
				var obj = _runtime_objectives[i]
				if obj._quest_objective_instance.is_sequential() and obj._quest_objective_instance.get_order_index() == next_index:
					obj.set_active(true)
					objective_activated.emit(i)
					break
